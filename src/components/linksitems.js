...
function LinksItems (props) {
			...
			return (
				...
				<LinksOuter key={props.pageData.drupal_id} heading={props.heading} headingLevel={props.headingLevel} description={props.description}>
					{props.pageData.map (paragraph  => {
						...
						const urlLink = (contentExists(paragraph.field_link_url.uri)) ? (paragraph.field_link_url.uri.includes("entity:node/")) ? aliasData[paragraph.field_link_url.uri.replace("entity:node/","")]:
							(paragraph.field_link_url.uri.includes("internal:/")) ? ("/") : (paragraph.field_link_url.uri.includes("<nolink>")) ? null :
							 paragraph.field_link_url.uri :null;
						// set heading level to one lower based on the heading level of the header, if header does not exists set to h2
						const nextHeadingLevel = (props.displayType ==='grid')? (contentExists(props.heading))? (props.headingLevel === "h2")? "h3": (props.headingLevel==="h3") ? "h4" : "h5": "h2": ''
	...
	} else if(props.displayType==='small-grid') {
		 if(contentExists(props.pageData) && props.pageData.length !== 0){
			 return (
				<LinksOuter key={props.pageData.drupal_id} heading={props.heading} headingLevel={props.headingLevel} description={props.description}>
					{props.pageData.map (paragraph  => {
						...
						const urlLink = (contentExists(paragraph.field_link_url.uri)) ? (paragraph.field_link_url.uri.includes("entity:node/")) ? aliasData[paragraph.field_link_url.uri.replace("entity:node/","")]:
							(paragraph.field_link_url.uri.includes("internal:/")) ? ("/") : (paragraph.field_link_url.uri.includes("<nolink>")) ? null :
							 paragraph.field_link_url.uri :null;
						// set heading level to one lower based on the heading level of the header, if header does not exists set to h2
						const nextHeadingLevel = (props.displayType !=='list')? (contentExists(props.heading))? (props.headingLevel === "h2")? "h3": (props.headingLevel==="h3") ? "h4" : "h5": "h2": ''
						return <LinksElement key={paragraph.drupal_id} 
									url={urlLink} 
									image={imageFile} 
									headingLink={paragraph.field_link_url.title}
				 ...
				 <ul>
                    {props.pageData.map (paragraph  => {
                        if(contentExists(paragraph.relationships)){
							...
							console.log(props.headingLevel, "heading Level")
                            return(
                                <li key={paragraph.drupal_id}>
                                        <Link to={urlLink}>{paragraph.field_link_url.title}</Link>
                                </li>
                            )
                        }
                        return null;
