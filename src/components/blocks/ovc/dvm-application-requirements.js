          ...
          <Overlay.GatsbyImage gatsbyImageData={getImage(yamlFiles[yamlMap.image.src])} alt={yamlMap.image.alt}>
            <Container className="page-container">
              <Row className="h-100 w-100 p-5 justify-content-center align-items-center">
                  <p className="display-2 text-light"> <i class="fa-light fa-circle-1 fa-lg"> </i> <strong>{yamlMap.title}</strong></p>
              </Row>
            </Container>
          </Overlay.GatsbyImage>
        </div>
...
const query = graphql`
  query {
    blockContentYamlBlock(field_yaml_id: {glob: "dvm_application_requirements"}) {
      id
      field_yaml_id
      field_yaml_map
      relationships {
...
export default function DVMApplicationRequirements() {
