...
function chooseFooter(footer) {
  let footerSelection = ``;
  footer.node.relationships?.field_tags.forEach((tag) => {
    if (tag.__typename === "taxonomy_term__units" && tag.name.includes("Ontario Veterinary College")) {
      footerSelection = "OVC";
    }
  });
...
const CustomFooter = (props) => {
  ...
  switch (chooseFooter(footer)) {
    ...
    default:
      return <DefaultFooter logos={footerLogos} text={footerText} widgets={footerWidgets} />;
  }
};
