		...
		return (props.widgetData.map(mediatext => {			
			...
			if (mediatext.__typename==="paragraph__media_text") {
				...
				if (contentExists(mediatext.relationships.field_media_text_media.field_media_oembed_video)) {
					...
					/* const videoTranscript = (mediatext.relationships.field_media_text_media.relationships !== undefined && mediatext.relationships.field_media_text_media.relationships.field_media_file !== undefined ? mediatext.relationships.field_media_text_media.relationships.field_media_file.localFile.publicURL : "nope"); */
					
					return (<>
					<h2>Video {mediaTitle} Exists</h2>
					<Video videoSrc={videoID} videoTitle={mediaTitle} />
					<p>Video URL is {videoURL} and video ID is {videoID} and video transcript is test</p>
					</>)
