        ...
        if (typeof customElements.get('uofg-header') != 'function') {
            class UofGDropdownMenu extends HTMLElement {
                ...
                static get observedAttributes() {
                    return ['data-closeonblur', 'data-animation'];
                }
                render() {
                ...
                fadeIn() {
                    let time = this.getAttribute('data-transitiontime');
                    time = (time) ? time : '200';
                    ...
                    this.container.style.transition = `opacity ease-in ${(time) ? time : '200'}ms`;
                ...
                slideUp() {
                    ...
                    this.container.style.transition = `height ease-in ${time}ms, margin ease-in ${time}ms, padding ease-in ${time}ms`;
                ...
                slideDown() {
                    this.container.calculatedRenderHeight = this.getContainerHeight() + 'px';
                    let time = this.getAttribute('data-transitiontime');
                    time = (time) ? time : '200';
                    ...
                    //Wait for a repaint, as this.getContainerHeight has css updates, and if we dont wait below style changes will be added at the wrong time.
                    window.requestAnimationFrame(() => {
                        window.requestAnimationFrame(() => {
                            this.container.style.overflow = 'hidden';
                            this.container.style.paddingTop = 0;
                            ...
                            //Wait for a repaint, otherwise above style changes would be overwritten
                            window.requestAnimationFrame(() => {
                                window.requestAnimationFrame(() => {
                                    this.container.style.height = this.container.calculatedRenderHeight;
                ...
                getContainerHeight() {
                    let prevStyle = this.container.style.cssText;
                    let height;

                    this.container.style = 'position:absolute; visibility:hidden; display:block;';
                    height = this.container.offsetHeight;
                    this.container.style = prevStyle;
            ...
            class UofGHeader extends HTMLElement {
                    ...
                    if (this.isConnected && !this.content_container) {
                        ...
                        window.addEventListener('resize', () => {
                            if (window.matchMedia("(max-width:1024px)").matches) {
                                ...
                                this.reduce();
                                this.main_content.dispatchEvent(new Event('transitionend')) //On page load transitionend does not fire, so it needs to be manually fired otherwise header content will be hidden
                            } else {
                ...
                render() {
                    ...
                    flex_break.innerHTML = '<h2 class="sr-only">Main Menu</h2>';
                    ...
                    flex_break.innerHTML = '<h2 class="sr-only">Page Menu</h2>';
                    this.main_content.appendChild(flex_break);
                ...
                createResourcesMenu() {
                    ...
                    <div id="uofg-header-resources-menu-main-menu-container"></div>
                    <div id="uofg-header-resources-menu-page-specific-menu-container"></div>
                ...
                createSearchMenu() {
                    ...
                    menu.container.innerHTML = '<h2>Search Menu</h2> <div id="uofg-header-search-menu-search-bar-container"></div>';

                    fetch('/scripts/popularQueries') //GET popular search queries
                        .then((response) => {
                            if (response.ok) {
                ...
                createMainMenu() {
                    ...
                    futureStudents.container.innerHTML = `
                        <ul id="uofg-header-faculty-staff-links">
                            <li><a title="Student Services" href="https://www.uoguelph.ca/students/">Student Services</a></li>
                            <li><a title="Undergraduate Admissions" href="https://admission.uoguelph.ca/">Grad Admissions</a></li>
                            <li><a title="Graduate Admissions" href="https://graduatestudies.uoguelph.ca/">Undergrad Admissions</a></li>
                            <li><a title="What's your student profile?" href="https://sites.uoguelph.ca/categories/">What's your student profile?</a></li>
                            <li><a title="Admission requirements" href="https://admission.uoguelph.ca/admissionreqs">Admission requirements</a></li>
                            <li><a title="International Students" href="https://www.uoguelph.ca/international/students/international-students">International Students</a></li>
                            <li><a title="Apply at U of G" href="https://www.uoguelph.ca/applying">Apply at U of G</a></li>
                        </ul>
                        `;
                        ...
                        if (this.reduceTimer == null) {
                            this.reduceTimer = setTimeout(() => {
                                if (this.classList.contains('reduced')) {
                                    ...
                                    //Hide the flex break elements and the headers in them.
                                    this.main_content.querySelectorAll('.break').forEach((flex_break) => {
                                        flex_break.style.display = 'none';
                                    });

                                    //Make all dropdowns in the main menu and page specific stop closing on blur
                                    ...
                                    //Moving page specific and main menu into their containers within resources menu
                                    let mainMenuContainer = this.resourcesMenu.container.querySelector('#uofg-header-resources-menu-main-menu-container');
                                    let pageSpecificContainer = this.resourcesMenu.container.querySelector('#uofg-header-resources-menu-page-specific-menu-container');
                                    mainMenuContainer.appendChild(this.mainMenu);
                                    pageSpecificContainer.appendChild(this.pageSpecific);
                                    mainMenuContainer.style.display = '';
                                    pageSpecificContainer.style.display = '';

                                    //Moving search bar into it's container within search menu
                                    let searchBarContainer = this.searchMenu.container.querySelector('#uofg-header-search-menu-search-bar-container');
                                    searchBarContainer.appendChild(this.searchBar);

                                    //Make search menu visible
                                    this.searchMenu.style.display = '';

                                    //Relabel the resources menu 
                                    this.resourcesMenu.opener.textContent = 'Menu';
                                    this.resourcesMenu.opener.setAttribute('aria-label', 'Main Menu');

                                    //Add the reduce to main_content so all the approriate styles apply and we know that content is currently in the reduced state
                                    this.main_content.classList.add('reduced');
                                    this.main_content.classList.remove('expanded');

                                    //Add the main content back into the DOM
                                    this.content_container.appendChild(this.main_content);
                        ...
                        if (this.expandTimer == null) {
                            this.expandTimer = setTimeout(() => {
                                if (!this.classList.contains('reduced')) {
                                    ...
                                    //Reveal the flex break elements.
                                    this.main_content.querySelectorAll('.break').forEach((flex_break) => {
                                        flex_break.style.display = '';
                                    });

                                    //Make all dropdowns in the main menu and page specific start closing on blur
                                    this.mainMenu.querySelectorAll('uofg-dropdown-menu').forEach((dropdown) => {
                                    ...
                                    //Hide the page specific and main menu containers within resources menu
                                    let mainMenuContainer = this.resourcesMenu.container.querySelector('#uofg-header-resources-menu-main-menu-container');
                                    let pageSpecificContainer = this.resourcesMenu.container.querySelector('#uofg-header-resources-menu-page-specific-menu-container');
                                    mainMenuContainer.style.display = 'none';
                                    pageSpecificContainer.style.display = 'none';

                                    //Move page specific and main menu to their appropriate positions
                                    ...
                                    //Move search bar back to its proper place
                                    ...
                                    //Remove the reduce class on main_content so all the approriate styles apply and we know that content is currently in the expanded state
